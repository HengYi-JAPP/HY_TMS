	function validateTraderPrice(isSubmit){ 
		...
		if((priceStart==null||priceStart=='')||(priceEnd==null&&priceEnd=='')){	
			isCorrect=false;
		}else{
			isCorrect=tester.test(priceStart);
			isCorrect=isCorrect?tester.test(priceEnd):isCorrect;
		}
		...
		if(allowSubmit==true&&isSubmit){
			...
			document.getElementById("traderPriceForm").submit();
		}
	}
	...
	function validateBatchTraderPrice(isSubmit,switchOperation){ 
		var priceType = document.getElementById("priceType").value;		
		var priceUnit = document.getElementById("priceUnit").value;			
		var priceStart = document.getElementById("priceStart").value;				
		var priceEnd = document.getElementById("priceEnd").value;	
		...
		priceTip.setAttribute("color",isCorrect==true?"green":"red");
		isCorrect=true;
		if(checkBatchPrice("batchCustomerPrice",allowSubmit)==false){
			isCorrect=false;
			alertTip("没有输入运价不能保存。请在列表中输入运价。");
		}
		allowSubmit=allowSubmit==true&&isCorrect==true?true:false;
		if(allowSubmit==true&&isSubmit){
			
			if(freightModel&&freightModel=='1'){
				document.getElementById("operation").value="addBatchSingleTraderPrice";
			}
			if(freightModel&&freightModel=='2'){
				document.getElementById("operation").value="addBatchAllTraderPrice";
			}	
	...
	function checkBatchPrice(customerPricesName,allowSubmit){
		...
		for(var i=0;i<customerPrices.length;i++){
				if(customerPrices[i].value==null||customerPrices[i].value==''){
