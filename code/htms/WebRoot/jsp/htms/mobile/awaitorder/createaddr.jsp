...
String codetailId = request.getParameter("codetailId");
String customerId = request.getParameter("customerId");
String customerName= request.getParameter("customerName");

String isModify = (String)request.getParameter("isModify");
  ...
  <body> 
    <div class="container">
  	<div class="top">
  		<%
  		 ...
  		 %>
   			<div class="top-left"><a href="contoller.do?operation=applyOrder&corderId=<%=corderId %>&customerCode=<%=customerCode %>&customerId=<%=customerId %>&codetailId=<%=codetailId %>&userId=<%=userId %>&customerName=<%=customerName %>"><img src="jsp/htms/mobile/image/button/return.jpg" style="width: 25px"/></a></div>
   		<%}else{%>
   		<div class="top-left"><a href="contoller.do?operation=listCustMobileAddr&corderId=<%=corderId %>&customerCode=<%=customerCode %>&customerId=<%=customerId %>&codetailId=<%=codetailId %>&userId=<%=userId %>&customerName=<%=customerName %>"><img src="jsp/htms/mobile/image/button/return.jpg" style="width: 25px"/></a></div>
   		<%} %>
   			<div class="top-center">新建地址</div>
   	</div>
   	...
   	<div class="content">
   		<div class="edit_addr" >
   			...
   			<div class="edit_addr_right">
   				<input type="text" class="edit_addr_input" placeholder="填写联系人" id="input_name"/>
   			</div>
   		</div>
   		...
   		<div class="edit_addr" >
   			...
   			<div class="edit_addr_right">
   				<input type="text" pattern="[0-9]*" class="edit_addr_input" placeholder="填写联系方式" id="input_contact"/>
   			</div>
   		</div>
   		...
   		<div class="edit_addr" >
   			...
   			<div class="edit_addr_right">
   				<input input type="text" class="edit_addr_input" readonly="true" unselectable="on" onfocus="this.blur()"  placeholder="选择省" id="picker_province"/>
   		...
   		<div class="edit_addr" >
   			...
   			<div class="edit_addr_right">
   				<input type="text" class="edit_addr_input" readonly="true" unselectable="on" onfocus="this.blur()"  placeholder="选择市" id="picker_city" />
   				<span><img src="jsp/htms/mobile/image/button/left.jpg" class="custimg"></img></span>
   			</div>
   		</div>
   		...
   		<div class="edit_addr" >
   			...
   			<div class="edit_addr_right">
   				<input type="text" class="edit_addr_input" readonly="true" unselectable="on" onfocus="this.blur()" placeholder="选择县/区" id="picker_area" />
   				<span><img src="jsp/htms/mobile/image/button/left.jpg" class="custimg"></img></span>
   			</div>
   		</div>
   		...
   		<div class="edit_addr" >
   			...
   			<div class="edit_addr_right">
   				<input type="text" readonly="true" unselectable="on" onfocus="this.blur()" class="edit_addr_input" placeholder="选择街道" id="picker_street" />
   				<span><img src="jsp/htms/mobile/image/button/left.jpg" class="custimg"></img></span>
   			</div>
   		</div>
   		...
   		<div class="edit_addr" >
   			...
   			<div class="edit_addr_right">
   				<input type="text" class="edit_addr_input" placeholder="填写详细地址" id="input_addr_detail"/>
   			</div>
   		</div>
    ...
    function getAreaChild(nodeId, areaLevel) {
   	 ...
   	 var phoneTest = "^[0-9]*$";
   	 	...
   	 	if (selStreetId == -1 || selAreaCode == null) {
   	 	...
   	 	var params = {operation:'addCustAddrSave', areaId:areaId, areaCode:selAreaCode, customerId:<%=customerId %>,caddrDetail:addrDetail,userId:<%=userId %>,createTime:curTime,caddrContactor:name,caddrTelephone:contact};
   	 	Ajax.get(basePath, params, function(msg){	
                  		var msgObj = JSON.parse(msg);
                  		if ( parseInt(msgObj.isSuccess) == 1) {
                  			if(isModify == 0){
                  				window.location.href = basePath + "?operation=applyOrder&corderId=<%=corderId %>&customerCode=<%=customerCode %>&customerId=<%=customerId %>&codetailId=<%=codetailId %>&userId=<%=userId %>&customerName=<%=customerName %>";
                  			}else{
                  				window.location.href = basePath + "?operation=listCustMobileAddr&corderId=<%=corderId %>&customerCode=<%=customerCode %>&customerId=<%=customerId %>&codetailId=<%=codetailId %>&userId=<%=userId %>&customerName=<%=customerName %>";
                  			}
                  		}else {
                  			//alert("保存失败，请稍后再试");
