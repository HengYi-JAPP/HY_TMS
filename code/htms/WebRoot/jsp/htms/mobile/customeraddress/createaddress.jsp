  ...
  <body> 
    <div class="container">
  	<div class="top">
   			...
   			<div class="top-left"><a href="contoller.do?operation=applyMobile&codetailId=<%=codetailId %>&corderId=<%=corderId %>&customerCode=<%=customerCode %>"><img src="jsp/htms/mobile/image/button/return.jpg" style="width: 30px"/></a></div>
   			<%}
				if(isModify.equals("1")){ %>
   			<div class="top-left"><a href="contoller.do?operation=listMineAddr"><img src="jsp/htms/mobile/image/button/return.jpg" style=" width: 25px"/></a></div>
   			<%} %>
   			<div class="top-center">新建地址</div>
   			<div class="top-right" style="top:1%;"><a href="javascript:saveAddr()"><img src="jsp/htms/mobile/image/button/true.jpg" style="width: 30px;"/></a></div>
   	</div>
   	...
   	<div class="content">
   		...
   		<div class="edit_addr" >
   			...
   			<div class="edit_addr_right">
   				<input input type="text" class="edit_addr_input" readonly placeholder="选择省" id="picker_province"/>
   			</div>
   		</div>
   		...
   		<div class="edit_addr" >
   			...
   			<div class="edit_addr_right">
   				<input type="text" class="edit_addr_input" readonly placeholder="选择市" id="picker_city" />
   			</div>
   		</div>
   		...
   		<div class="edit_addr" >
   			...
   			<div class="edit_addr_right">
   				<input type="text" class="edit_addr_input" readonly placeholder="选择县/区" id="picker_area" />
   			</div>
   		</div>
   		...
   		<div class="edit_addr" >
   			...
   			<div class="edit_addr_right">
   				<input type="text" readonly class="edit_addr_input" placeholder="选择街道" id="picker_street" />
   			</div>
   		</div>
    ...
    function getAreaChild(nodeId, areaLevel) {
   	 ...
   	 function saveAddr() {
   	 	...
   	 	if (contact == null || contact.length == 0) {
   	 		$.DialogByZ.Alert({Title: "提示", Content: "请填写联系方式",BtnL:"确定",FunL:alerts});
   	 		return;
   	 	}
   	 	...
   	 	var params = {operation:'addAddrSave', areaId:areaId,customerId:<%=customerId %>,caddrDetail:addrDetail,userId:<%=userId %>,createTime:curTime,caddrContactor:name,caddrTelephone:contact};
   	 	Ajax.get(basePath, params, function(msg){	
                  		var msgObj = JSON.parse(msg);
                  		if ( parseInt(msgObj.isSuccess) == 1) {
                  			<%if(isModify.equals("0")){ %>
                  				window.location.href = basePath + "?operation=applyMobile&codetailId=<%=codetailId %>&corderId=<%=corderId %>&customerCode=<%=customerCode %>";
   							<%}
