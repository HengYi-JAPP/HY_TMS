...
<html:form id="custOrderForm"  enctype="application/x-www-form-urlencoded"  action="contoller.do"  method="GET" >
	...
	<%
	  //System.out.println(" customerCode  customerCode: isTraders:   "+isTraders+"   "+isFetch);
	if(isTraders==1&&isFetch==true){ %>
	<html:table id="1667" styleClass="edit-talbe" >
		<html:tr styleClass="trCell">
			<td class="edit-talbe-label">
				是否自提
			...
			-->
				...
				<%
				  ...
				  // System.out.println(" customerCode  customerCode: defaultFetch"+defaultFetch);
				   if(defaultFetch){
				   %>
				       <html:radio id="selfFetchOff" name="selfFetch" value="0"  onchange="showOrHideOrderDetail('selfFetchOff','showFecthInfo');"/>否
				       <html:radio id="selfFetchOn" name="selfFetch" value="2" checked="checked" onchange="showOrHideOrderDetail('selfFetchOn','showFecthInfo');"/>是
				   <%
				   }else{
				   %>
				       <html:radio id="selfFetchOff" name="selfFetch" value="0" checked="checked" onchange="showOrHideOrderDetail('selfFetchOff','showFecthInfo');"/>否
			       	   <html:radio id="selfFetchOn" name="selfFetch"  value="1" onchange="showOrHideOrderDetail('selfFetchOn','showFecthInfo');"/>是
				   <%
				   }
				 %>
	...
	<html:table id="showFecthInfo" styleClass="edit-talbe"    style="display:none;">
	             <%
				   ...
				   %>
				       <script type="text/javascript">
				           showOrHideOrderDetail('selfFetchOn','showFecthInfo');
				       </script>
				    <%
				   }
				 %>            
		...
		<html:tr styleClass="trCell">
			...
			<html:td colspan="3">
			    <% 
			       if(defaultFetch){
			    %>
				    <html:select  id="fetchCar"  name="fetchCar"  list="selfFetchCarList"  >
					    <html:option  value="vehicleNo"  label="vehicleNo" ></html:option>
					</html:select>
			    <% 
			    }else{
			    %>
	...
	<html:table list="orderDetailList" bean="detailBean" styleClass="dgbackground" cellSpacing="0"  border="1" id="userGrid_DataGrid1" style="width:100%; border-collapse: collapse;" align="center" isHead="true" >
		<html:tr id="applyTrInfor$[detailBean.codetailCertificate]" headerStyleClass="dgHeaderStyle" styleClass="dgAlternatingItemStyle" >
			<html:td label="行号" property="codetailCertificate" style="width:70px;" width="70px" >

			</html:td>	
			<html:td label="工厂" property="factoryName"   width="60px"/>
			<html:td label="产品" property="productName"   width="60px"/>	
