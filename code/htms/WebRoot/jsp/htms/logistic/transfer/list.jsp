...
<html:form id="transferForm"  enctype="application/x-www-form-urlencoded"  action="contoller.do"  method="GET" loadCache="true">
	<com:loader  simplendar="true"  ajax="true" asyncSelect="true" />
	<html:hidden id="operation" name="operation" value="listTransfer"/>
	<com:loader binder="true" checkall="true" switchbox="true" ajax="true" />
	<jsp:include page="../../../common/impart/table-banner-new.jsp">
		<jsp:param value='<%=tools.tool.getLocal("jsp.htms.logistic.transfer.title.list") %>' name="title"/>
	</jsp:include>
	
	
	<table id="1706" class="edit-talbe" style="width:1523px;"  >
	    ...
	    <tr class="trCell">
		 ...
		 <tr class="trCell" style="width:100%" >
		      ...
		      <td width="212px;">
		      	<jsp:include page="../../export/exportTransfer.jsp">
					<jsp:param value='' name=""/>
				</jsp:include>
		      </td>
		      <td    id="searchBox" colSpan="6" rowSpan="1"  align="left" >
		      		
		           <btn:search id="searchTransfer" href="javascript:submit('transferForm')"/>
				   <btn:reset id="resetTransfer" href="javascript:cleanForm('transferForm')"/>
		     </td>
		 </tr>
		 
	</table>
	<table class=" background" cellSpacing="0"  border="1" id="userGrid_DataGrid1" style="width:100%;text-align:left;overflow-x:scroll;overflow-y:hidden;float:left;border-collapse: collapse;margin-top: 10px;">
		<tr  class="headerStyle" >
	    	<td  width="100px" colspan="3">操作</td>
	    	<td width="40px">加急</td>
		...
		for(int i=0,size=transferKeyList.size();i<size;i++){
			detailList=transferDetailListMap.get(transferKeyList.get(i));
				for(int d=0,dsize=(detailList==null?0:detailList.size());d<dsize;d++){
					...
					String customerAccount=transBean.getCustomerAccount();
					customerAccount=(customerAccount==null||customerAccount=="")?"0":customerAccount;
					BigDecimal customerAmount = new BigDecimal(customerAccount);
				    
				    String customerName = transBean.getCustomerName();
				    if(customerAmount.compareTo(new BigDecimal("0"))>=0){
				    
				    }else{
				        customerName="<font color='red' >"+customerName+"</font>";
				    }
					...
					<tr class="alternatingItemStyle"   onclick="onMouseMoveOverTableList(this)">
						<%if(d==0){
						 %>
						                ...
						                <td  rowspan="<%=dsize %>">
											<%if(transStatus==0){//没有分配承运商的可以分拆 %>
												<html:a href="contoller.do?operation=toTransferSplit&corderId=$[transferBean.corderId]&codetailId=$[transferBean.codetailId]&transId=$[transferBean.transId]&transDetailId=$[transferBean.transDetailId]&caddrId=$[transferBean.caddrId]" style="border:0;text-decoration:none;">
												    <!-- <input type="button"  id="" name="" value="分拆"  style="width:65px;height:24px;background-image:url(resources/images/buttons/merger-dossier.gif);border-style:none;background-repeat:no-repeat;background-color:transparent;padding-left:25px;color:#1b4f9a;font-weight:bold;font-size:12px;cursor:pointer;">
												     -->
												    <img src="images/htms/transfer/split.png"  title="分拆" height="20" width="20" />
												</html:a>
											<%} %>
										</td>
										<td  rowspan="<%=dsize %>">
												<%if(transStatus==0){//没有分配承运商的可以分拆 %>
												<html:a href="contoller.do?operation=toAssignTransfer&&corderId=$[transferBean.corderId]&codetailId=$[transferBean.codetailId]&transId=$[transferBean.transId]&transDetailId=$[transferBean.transDetailId]&caddrId=$[transferBean.caddrId]" style="border:0;text-decoration:none;">
												<!-- <input type="button"  id="" name="" value="分配"  style="width:65px;height:24px;background-image:url(resources/images/buttons/pigeonhole.gif);border-style:none;background-repeat:no-repeat;background-color:transparent;padding-left:25px;color:#1b4f9a;font-weight:bold;font-size:12px;cursor:pointer;">
												 -->
												     <img src="images/htms/transfer/assign.png" title="分配"  height="20" width="20" />
												</html:a>
												<%} %>
												<%if(transStatus==1){//分配承运商未排车的可以取消 %>
												   <!--<btn:cancel  href="contoller.do?operation=cancelTransferTransport&transId=$[transferBean.transId]&transShipId=$[transferBean.shipId]"  />-->
												  <html:a href="contoller.do?operation=cancelTransferTransport&transId=$[transferBean.transId]&transShipId=$[transferBean.shipId]">
												       <img src="images/htms/transfer/cancel.png"  title="取消"  height="20" width="20" />
												  </html:a>
												<%} %>
										</td>
										<td rowspan="<%=dsize %>">
											<%if(transStatus==0&&isDefault!=1){//没有分配承运商的可以删除 %>
												<!-- <btn:delete  href="contoller.do?operation=deleteSplitTransfer&currentTransId=$[transferBean.transId]&transShipId=$[transferBean.shipId]"  />
											    -->
											    <cext:priv code="_htms_split_manage_delete">
											     <html:a href="contoller.do?operation=deleteSplitTransfer&currentTransId=$[transferBean.transId]&transShipId=$[transferBean.shipId]">
											          <img src="images/htms/transfer/delete.png"  title="删除"  height="20" width="20" />
											     </html:a>
											    </cext:priv>
											<%} %>
											<%if(transStatus==1||transStatus==2){//分配承运商未排车的可以取消 %>
												   <html:a href="contoller.do?operation=trackTransfer&transId=$[transferBean.transId]&returnOperation=listTransfer">
										...
										<td style="text-align:center;" rowspan="<%=dsize %>">
										    <html:a style="text-decoration:none;" href="contoller.do?operation=toViewAndModifyTransfer&corderId=$[transferBean.corderId]&codetailId=$[transferBean.codetailId]&transId=$[transferBean.transId]&transDetailId=$[transferBean.transDetailId]&caddrId=$[transferBean.caddrId]">
										       <%=shipCode %>
										    </html:a>
										</td>
	...
	<table id="pageFooter" class="">
		<tr class="trCell">
			<td   class="pagination"   id="pagination" >
				...
				<com:page formId="transferForm" pageCtlName="pageCtl"/>
			</td>
		</tr>
	</table>
</html:form>
