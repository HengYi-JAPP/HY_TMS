...
<com:loader css="css/common/sub-table-list.css" switchbox="true"/>
	...
	<% 
	   ...
	   List<FreightBean> sapFreightList =(List<FreightBean>) request.getAttribute("sapFreightList");
       sapFreightList=sapFreightList==null?new ArrayList<FreightBean>():sapFreightList;
	   if(sapFreightList!=null&&sapFreightList.size()>0){
	        int sapFreightSize= sapFreightList.size();
	   %>
	...
	<table  class="dgbackground" cellSpacing="0"  border="1" id="userGrid_DataGrid1" style="width:100%; border-collapse: collapse;" align="center">
	    <tr style="height:15px;"  height="25px"  class="dgHeaderStyle" >
	        <td  width="50px" >计费方式</td>
	        <td  width="50px" >行号</td>
	        <td width="65px"  >申请数量</td>	
			<td width="40px"  >计价方式</td>	
			<td width="40px"  >价格</td>
			 <td width="60px" >运费</td>		
	        <td width="60px" >运费创建时间</td>
	    </tr>
	    <%
	    FreightBean totalFreightBean = sapFreightList.get(0);
	    BigDecimal totalApply= new BigDecimal(0);
	    for(FreightBean  freightBean:sapFreightList){
	        totalApply=totalApply.add(new BigDecimal(freightBean.getApplyTonNum()==null?"0":freightBean.getApplyTonNum()));
	        String freightType =  freightBean.getFreightType()==null?"":freightBean.getFreightType().trim();
	    %>
	     <tr  style="height:30px;" >
                     <td style="text-align:center;" >						
						<%="1".equals(freightType)?"正常运费":"" %>
						<%="2".equals(freightType)?"客户运价":"" %>
						<%="3".equals(freightType)?"指定运价":"" %>
						<%="4".equals(freightType)?"贸易商运价":"" %>
						<%="5".equals(freightType)?"自备车自提":"" %>
					</td>
					<td style="text-align:center;" >
						<%=freightBean.getItemCode()    %>
					</td>
				     <td style="text-align:center;" >
				        <%=freightBean.getApplyTonNum() %>
					</td>
					<td style="text-align:center;" >
					    <%=freightBean.getFreightModel() %>
						
					</td>
					<td style="text-align:center;" >
						<%=freightBean.getFreightPrice() %>（元/<%=freightBean.getFreightStyle().equals("6")?"吨":"" %>
						<%=freightBean.getFreightStyle().equals("24")?"千克":"" %>）
					</td>
					<td style="text-align:center;" >
				       <%=freightBean.getFreightAmount() %>
					</td>
					<td style="text-align:center;" >
						<%=freightBean.getFreightTime() %>
					</td>
	          </tr>
	    <%
	    }
	     %>
	      ...
	      <tr height="30px">
		     ...
		     <td  style="text-align:center;">  <%=totalApply %></td>
		     <td  style="text-align:right;"> </td> 
		     <td  style="text-align:right;"> </td> 
		     <td  style="text-align:center;"> <%=totalFreightBean.getTotalFreight() %></td> 
		     <td  style="text-align:right;"> </td> 
		 </tr> 
	</table>
...
<logic:if condition="$[tranferViewBean.transStatus]==2">
	...
	<html:table  cellSpacing="0"  align="center" isHead="false" >
		<html:tr >
			<html:td >
			  <!--    $[tranferViewBean.transStatus]    
			          $[tranferViewBean.isDefault]  -->
			    <logic:if condition="$[tranferViewBean.transStatus]==0">
			    	 <logic:if condition="$[tranferViewBean.isDefault]!=1">
			    	      <btn:modify href="contoller.do?operation=toViewAndModifyTransfer&isModify=1&curPage=$[curPage]&corderId=$[tranferViewBean.corderId]&codetailId=$[tranferViewBean.codetailId]&transId=$[tranferViewBean.transId]&transDetailId=$[tranferViewBean.transDetailId]&caddrId=$[tranferViewBean.caddrId]"/>
			        </logic:if>
			    </logic:if>		
				
				<btn:return href="contoller.do?operation=listTransfer&curPage=$[curPage]"/>
			</html:td>
		</html:tr>
	</html:table>
