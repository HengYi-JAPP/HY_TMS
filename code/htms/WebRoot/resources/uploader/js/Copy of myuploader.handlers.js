function fileQueueError(file, errorCode, message) {
	try {
		...
		var errorName = "";
		if (errorCode === MyUploader.errorCode_QUEUE_LIMIT_EXCEEDED) {
		...
		switch (errorCode) {
		case MyUploader.QUEUE_ERROR.ZERO_BYTE_FILE:
			imageName = "<font color='red'>文件大小为0</font>";
			break;
		case MyUploader.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
			imageName = "<font color='red'>文件大小超过限制</font>";
			break;
		case MyUploader.QUEUE_ERROR.ZERO_BYTE_FILE:
		case MyUploader.QUEUE_ERROR.INVALID_FILETYPE:
		default:
			alert(message);
			break;
		}
 ...
 * @param {} file
 * id : string,			    // MyUploader控制的文件的id,通过指定该id可启动此文件的上传、退出上传等
 ...
 * filestatus : number		// 文件的当前状态，对应的状态代码可查看MyUploader.FILE_STATUS }
 */
...
function uploadError(file, errorCode, message) {
	...
	try {
		switch (errorCode) {
		case MyUploader.UPLOAD_ERROR.FILE_CANCELLED:
			try {
			...
			break;
		case MyUploader.UPLOAD_ERROR.UPLOAD_STOPPED:
			try {
				progress = new FileProgress(file,  this.customSettings.upload_target);
				progress.setCancelled();
		...
		case MyUploader.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
			imageName = "<font color='red'>文件大小超过限制!</font>";
			break;
		default:
			alert(message);
...
FileProgress.prototype.toggleCancel = function (show, MyUploaderInstance) {
	this.fileProgressElement.childNodes[0].style.visibility = show ? "visible" : "hidden";
	if (MyUploaderInstance) {
		var fileID = this.fileProgressID;
		this.fileProgressElement.childNodes[0].onclick = function () {
			MyUploaderInstance.cancelUpload(fileID);
			return false;
		};
	}
};
