...
function validateSplitTonNum(itemNo){
	...
	if(!reg.test(splitTonNum)){
			//alert("请输入当前分拆数，为整数或带3位小数的数");
			$.DialogByZ.Alert({Title: "提示", Content:"请输入当前分拆数，为整数或带3位小数的数",BtnL:"确定",FunL:alerts});
	...
	if(applyNum > initTon){
		//alert("当前分拆数不能大于可分拆数");
		$.DialogByZ.Alert({Title: "提示", Content:"当前分拆数不能大于可分拆数",BtnL:"确定",FunL:alerts});
		document.getElementById("splitTonNum"+itemNo).style.color="red";
		return false;
	}
	...
	if(totalSplitTon > 40000){
		//alert("请调整分拆数，分拆项累计的分拆数不能大于40吨.");
		$.DialogByZ.Alert({Title: "提示", Content:"请调整分拆数，分拆项累计的分拆数不能大于40吨.",BtnL:"确定",FunL:alerts});
		document.getElementById("splitTonNum"+itemNo).style.color="red";
...
function validateSplitPackNum(itemNo){
	...
	if(!reg.test(splitPackNum)){
			//alert("请输入当前分拆包数，包数需输入整数");
			$.DialogByZ.Alert({Title: "提示", Content:"请输入当前分拆包数，包数需输入整数",BtnL:"确定",FunL:alerts});
	...
	if(applyNum > initPack){
		//alert("当前分拆包数不能大于可分拆包数");
		$.DialogByZ.Alert({Title: "提示", Content:"当前分拆包数不能大于可分拆包数",BtnL:"确定",FunL:alerts});
		document.getElementById("splitPackNum"+itemNo).style.color="red";
		return false;
	}
	...
	document.getElementById("appliablePack"+itemNo).value=parseInt(newAppliedAmount);
	...
	document.getElementById("totalPack").value=parseInt(calculator.sub(initTotalPack,totalSplitPack));/*更新可分拆总吨数*/
	document.getElementById("splitTotalPack").value=parseInt(totalSplitPack);/*更新当前分拆总吨数*/
