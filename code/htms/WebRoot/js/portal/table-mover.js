 ...
 * 开始行下标
 */
var _rStart_rIndex_ = 1;
function moveUp(obj,params){//上移
	//alert("_rStart_rIndex_="+_rStart_rIndex_);
	...
	if(_rStart_rIndex_==orderNum-- || table.rows.length==_rStart_rIndex_+1)return;//table的第一行是标题，当table只有两行或者或当前行index为1时，不移动。
...
function moveTop(obj,params){//置顶
	...
	var temp=table.rows.item(_rStart_rIndex_);
...
function moveDown(obj,params){//下移
	...
	orderNum++;
	if(table.rows.length==orderNum || table.rows.length==_rStart_rIndex_+1)return;
